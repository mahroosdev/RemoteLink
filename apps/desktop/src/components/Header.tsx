    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        >
          ...
          {isStarting ? 'STARTING...' : engineActive ? 'STOP ENGINE' : 'START ENGINE'}
        </Button>
        <span style={{ fontSize: '12px', fontWeight: 600, color: online ? 'var(--accent-green)' : serverStatus === 'error' ? 'var(--accent-red)' : 'var(--text-muted)', letterSpacing: '0.5px' }}>
          {online ? 'ENGINE ONLINE' : isStarting ? 'STARTING' : serverStatus === 'error' ? 'ENGINE ERROR' : 'OFFLINE'}
        </span>
      </div>
    </div>
